...
def build_prompt(trigger: dict, horizon: dict, evidence: list[dict], market: dict) -> str:
    ...
    return (
        ...
        'DEPENDS_ON|EXPORTS_TO|CAPACITY_CONSTRAINED_BY|PRICED_IN|REGULATED_BY|PASSES_COST_TO, "confidence": 0-1, "lag_days": number, "evidence_ids": list of int (indices into the bundle, 0-based)}\n'
...
def append(rec: Record, store: Path | None = None) -> Path:
    path = store or STORE
